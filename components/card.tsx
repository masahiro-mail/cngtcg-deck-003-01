...
interface CardComponentProps {
  ...
  onClick?: () => void
  className?: string
  deckCount?: number // デッキ内の枚数を表示するためのプロパティを追加
}
...
export default function CardComponent({
  ...
  const formatCardName = () => {
    ...
    // 「奥義:」で始まる場合は特別処理
    if (name.startsWith("奥義:") && !noBreakNames.includes(name)) {
      return (
        <>
          <div>奥義:</div>
          <div>{name.substring(3)}</div>
        </>
      )
    }

    // コロンがある場合はコロンで分割
    if (name.includes(":") && !noBreakNames.includes(name)) {
    ...
    // スペースがある場合は適切な位置で分割
    if (name.includes(" ") && name.length > 8 && !noBreakNames.includes(name)) {
      const spaceIndex = name.indexOf(" ")
      // スペースの位置が名前の長さの半分より前にある場合
      if (spaceIndex < name.length / 2) {
        const secondSpaceIndex = name.indexOf(" ", spaceIndex + 1)
        if (secondSpaceIndex !== -1) {
          return (
            <>
              <div>{name.substring(0, secondSpaceIndex)}</div>
              <div>{name.substring(secondSpaceIndex + 1)}</div>
            </>
          )
        }
      }
  ...
  return (
    <div
      ...
      onClick={onClick}
      style={{
        boxShadow: "0 4px 10px rgba(0, 0, 0, 0.3), 0 0 4px rgba(255, 255, 255, 0.2) inset",
      }}
    >
      {/* カード背景 - グラデーションを追加 */}
      <div
      ...
      {card.colorCost > 0 && (
        <div
          ...
          style={{
            color: "#ffffff", // 白色
            textShadow: "0 1px 1px rgba(0, 0, 0, 0.7)", // 視認性のために影を追加
          }}
        >
          {card.colorCost}
        </div>
      ...
      <div
        ...
        style={{
          top: "calc(20px + 1px)", // 0.5pxからさらに0.5px下げて合計1pxに
          left: "calc(2.5rem + 0.5px)",
          color: "#000000", // 黒色
          textShadow: "0 1px 1px rgba(255, 255, 255, 0.5)", // 白い影を追加して視認性を確保
        }}
      >
        {card.colorlessCost}
      </div>
